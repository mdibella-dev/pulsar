...
let blobStore = null;
let NativeCompileCache = require("../src/native-compile-cache");

const testValue = "Hello World";

console.log(`Main Thread: ${testValue}`);

const onloadWorker = new Worker("onload_Worker.js", { type: "module"});

onloadWorker.onmessage = (e) => {
...
window.onload = () => {
  onloadWorker.postMessage({
    pathToLoad: path.join(process.env.ATOM_HOME, "blob-store"),
    processVersion: process.versions.v8,
  });
  ...
  try {
    ...
    //const onloadWorker = new Worker("onload_Worker.js");

    //onloadWorker.postMessage({
    //  pathToLoad: path.join(process.env.ATOM_HOME, "blob-store"),
    //  processVersion: process.version.v8
    //});

    //let NativeCompileCache;
    //onloadWorker.onmessage = (e) => {
    //  NativeCompileCache = e.NativeCompileCache;
    //  blobStore = e.blobStore;
    //};
